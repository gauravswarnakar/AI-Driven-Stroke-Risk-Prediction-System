...
import datetime  # Import datetime for dynamic filename generation
...
# Save predictions to CSV with a dynamic filename
timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")  # Format: YYYYMMDD_HHMMSS
dynamic_filename = f"patient-data-updated-{timestamp}.csv"
data['Predicted_at_risk'] = rf_model.predict(X)
data.to_csv(f"C:/Users/Gaurav/OneDrive/Desktop/{dynamic_filename}", index=False)

print(f"The dataset with predictions has been saved as '{dynamic_filename}'.")
